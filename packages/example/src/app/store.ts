...
export class LoadAction implements ActionType<StateModel, never> {
    ...
    async execute(ctx: StateContextType<StateModel>): Promise<StateModel> {
        ...
        if (currentState.users.length === 0) {
            ...
            const users = ((await (await fetch('https://randomuser.me/api/?results=20')).json()) as ApiResponse).results; //axios.get<ApiResponse>('https://randomuser.me/api/?results=20')).data.results;
